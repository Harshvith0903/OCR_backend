...
def paddle_ocr(image):
    ...
    # Convert OpenCV image to RGB format (required by PaddleOCR)
    ...
    extracted_text = " ".join([line[1][0] for res in result for line in res]) if result else ""
...
def process_pdf(pdf_path, output_excel_path):
    ...
    for page_number, img in enumerate(images, start=1):
        ...
        extracted_text = paddle_ocr(gray)
        ...
        for word in words:
            if re.fullmatch(main_pattern, word):  # Strict regex match
                ...
                # Ensure extracted tag is uppercase and trimmed
                if unique_tag:
                    unique_tag = unique_tag.strip().upper()
                    print(f"Extracted Unique Tag: {unique_tag}")  # Debugging

                    # Check if tag exists in the master file
                    if unique_tag in tag_mapping:
                        ...
                        print(f"Match Found! {unique_tag} -> {equipment_desc}, {department}")  # Debugging
                    else:
                        print(f"No Match Found for: {unique_tag}")  # Debugging
                        equipment_desc = "N/A"
                        department = "N/A"
                else:
                    equipment_desc = "N/A"
                ...
                extracted_data.append({
                    "Sl.no.": len(extracted_data) + 1,  # Serial number
                    "Discipline": department,  # Matched department
                    "Tag Number": word, 
                    "Tag Identifier Code": unique_tag,  # Extracted tag
                    "Equipment Description": equipment_desc,  # Matched equipment description
                    "Drawing Number": drawing_number,  # File name without extension 
...
def get_tags():
    ...
    return jsonify({"tags": df.to_dict(orient="records")})
