...
# Define OCRAD executable path (update this if needed)
OCRAD_PATH = "ocrad"  # Use full path if needed, e.g., "/usr/local/bin/ocrad"
...
def ocrad_ocr(image_path):
    """Runs OCRAD OCR on the given image file with proper encoding handling."""
    try:
        result = subprocess.run(["ocrad", image_path], capture_output=True, text=True, errors="replace")
...
def process_pdf(pdf_path, output_excel_path):
    ...
    for page_number, img in enumerate(images, start=1):
        ...
        extracted_text = ocrad_ocr(gray_image_path)
