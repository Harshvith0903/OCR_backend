...
def doctr_ocr(image):
    """Uses Doctr OCR to extract text from an image."""
    img_pil = Image.fromarray(image).convert("RGB")

    # Perform OCR using Doctr
    results = doctr_model([img_pil])

    # Extract text from Doctr results
    extracted_text = " ".join([word[0] for block in results.pages[0].blocks for word in block.lines])
